...
import numpy as np
import pandas as pd
import cmsn_python.examples.gui.mainwindow as mainwindow
...
class MyDeviceListener(CMSNDeviceListener):
    def __init__(self,queue_data):
        ...
        self.last_time_sent = 0  # Initialize time tracking variable
    ...
    def on_eeg_data(self, eeg_data):
        if main_window is not None:
            ...
            if main_window._collect_data:
                main_window._eeg_history.append([time.time(), eeg_data.eeg_data, main_window._meditation_history[-1]])
                ...
                df = pd.DataFrame(main_window._eeg_history, columns=['time_stamp', 'raw_eeg', 'meditation'])
    ...
    def on_meditation(self, meditation):
        ...
        self.queue_data.put(meditation)


    def get_meditation_label(self):
        # Return the latest meditation label stored in self
        return self.meditation_label
    ...
    def clicked_connect_button(self):
        ...
        for device in self._device_list:
            if device.name == device_name:
                ...
                self._selected_device.set_listener(MyDeviceListener(self.queue_data))
        # connect device
...
def run_bci_app(queue_data):
    app =QApplication(sys.argv)
    win = MainWindow(queue_data)
    win.show()
    sys.exit(app.exec_())
